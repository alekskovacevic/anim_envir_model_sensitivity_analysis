...
initial_state <- c(FishN = 100, FishCs = 20, FishCr = 1, # fish seeding
                   FarmN = 100, FarmCs = 19, FarmCr = 0, # farm seeding 
                   WildN = 100, WildCs = 20, WildCr = 0, 
                   PetN = 100, PetCs = 20, PetCr = 0,
                   WaterCs = 10e5, WaterCr = 0, # water seeding 
...
all5_wide_row <- ggplot(all_prcc_data, aes(x = Parameter, y = PRCC, fill = PRCC)) +
  ...
  labs(title = "Sensitivity Analysis Across Ecological Compartments (Fish Seeding)",
...
print(all5_wide_row)
ggsave("Sensitivity_Fish_Seeding.png", plot = all5_wide_row, height = 8, width = 15)
...
# keep selected parameters in original param_order 
...
print(sel5_wide_row)
ggsave("Sensitivity_Fish_Seeding_TOP10.png", plot = sel5_wide_row, height = 8, width = 15)
