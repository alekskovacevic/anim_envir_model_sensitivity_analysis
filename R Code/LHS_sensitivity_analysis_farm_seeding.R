...
initial_state <- c(FishN = 100, FishCs = 20, FishCr = 0, 
                   FarmN = 100, FarmCs = 19, FarmCr = 1, # farm seeding or 
...
all5_wide_row <- ggplot(all_prcc_data, aes(x = Parameter, y = PRCC, fill = PRCC)) +
  ...
  labs(title = "Sensitivity Analysis Across Ecological Compartments (Farm Seeding)",
...
print(all5_wide_row)
ggsave("Sensitivity_Farm_Seeding.png", plot = all5_wide_row, height = 8, width = 15)
...
print(sel5_wide_row)
ggsave("Sensitivity_Farm_Seeding_TOP10.png", plot = sel5_wide_row, height = 8, width = 15)
